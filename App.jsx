...
export default function EscanerBoletas() {
  ...
  async function procesarArchivos(files) {
    ...
    setProcesando(false);
    showToast(`✅ ${nuevas.length} boleta${nuevas.length > 1 ? "s" : ""} agregada${nuevas.length > 1 ? "s" : ""}`);
  }
    ...
    <div style={{ minHeight: "100vh", background: "#f0f4f8", fontFamily: "Inter, Arial, sans-serif", padding: "20px 14px" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          ...
          {boletas.length > 0 && (
            <div style={{ background: "#e8f5e9", borderRadius: 10, padding: "8px 14px", textAlign: "right" }}>
              <div style={{ fontSize: 11, color: "#388e3c", fontWeight: 600 }}>TOTAL ACUMULADO</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "#217346" }}>${total.toLocaleString("es-CL")}</div>
            </div>
          )}
        </div>
          ...
          <>
            <div style={{ background: "#fff", borderRadius: 14, overflow: "auto", boxShadow: "0 2px 12px rgba(0,0,0,0.07)", marginBottom: 14 }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                ...
                <tfoot>
                  <tr style={{ background: "#1a2540", color: "#fff", fontWeight: 700 }}>
                    <td style={{ ...td, color: "#fff" }} colSpan={3}>TOTAL ({boletas.length} boletas)</td>
                    <td style={{ ...td, color: "#7ee8a2", fontSize: 15 }}>${total.toLocaleString("es-CL")}</td>
                    <td colSpan={2}></td>
                  </tr>
                </tfoot>
              </table>
...
const th = { padding: "11px 13px", textAlign: "left", fontWeight: 600, fontSize: 12 };
const td = { padding: "9px 13px", color: "#2d3a55", verticalAlign: "middle" };
